...
namespace FactLab
{
    public partial class DetallesFactura : Page
    {
        //Properties
        ...
        {
            //todo select 
            fact = Request.QueryString["factura"];
            ...
            if (verificaStatus(fact))
            {
                ...
                if (SAD.Visible == false)
                {
                    ...
                    {
                        Response.Redirect("SAD.aspx?factura=" + fact);
                    }
                }
                else
                {
        ...
        public void iniciaDatos()
        {
            ...
            foreach (DataRow row in td.Rows)
            {
                ...
                if (uuidrel == null || uuidrel.Equals(row["uuidrel"].ToString())) { txtUUIDREL.Text = row["uuidrel"].ToString(); }
                else { txtUUIDREL.Text = uuidrel; }

                if (relac == null || relac.Equals(row["relacion"].ToString())) { txtRelacion.Text = row["relacion"].ToString(); }
        ...
        public void generaTXT()
        {
            ...
            using (System.IO.StreamWriter escritor = new System.IO.StreamWriter(path))
            {
                ...
                foreach (int item in detalle33.Keys) //Se recorren los registros del Hashtable Detalle33
                {
                    ...
                    else if (lista[15].ToString().Trim() == "0.00" && idreceptor == "CHROBMEX" && lista[3].ToString().Trim() != "Otros" && lista[3].ToString().Trim() != "refact" || lista[15].ToString().Trim() == "0.00" && idreceptor == "CHROBMEX" && sinImpuestos > 0)
                    {
                        sinImpuestos++;
                        escritor.WriteLine(
                    "041"                                                  //1-Tipo De Registro
                ...
                if (!campoIncorrecto)
                {
                    ...
                    if (salida != "FALLA AL SUBIR")
                    {
                        if (System.Web.Configuration.WebConfigurationManager.AppSettings["activa"].Equals("Si"))
                        {
                            ...
                            string Origenn = "1";
                            string Destino = "";
                            //facLabControler.generadas(folioFactura, serieFactura, uuidFactura, pdf_xml_descargaFactura, pdf_descargaFactura, xlm_descargaFactura, cancelFactura, lblFact.Text, fechaFinal, txtTotal.Text.ToString().Trim(), moneda, txtRFCDestino.Text.Trim(), Origenn, Destino);
                            facLabControler.generadas(mbnumber.ToString(), fact, serie, idreceptor, fechaFinal, txtTotal.Text.ToString().Trim(),
                               moneda, pdf_descargaFactura, xlm_descargaFactura, imaging, bandera, "Tralix", "VAL", ultinvoice, hecha, txtOrden.Text.Trim(), uuidFactura);
                        }
                        ...
                        //ScriptManager.RegisterStartupScript(this, GetType(), "swal", "swal('" + msg + "', 'Correcto ', 'success');setTimeout(function(){window.location.href ='Default.aspx'}, 10000)", true);
        ...
        public Hashtable generaActualizacion()
        {
            ...
            foreach (int item in datosTabla.Keys)
            {
                ...
                try
                {
                    total = double.Parse(list[5].ToString());
                }
                catch (Exception ex)
                {
                    string error = ex.Message;
